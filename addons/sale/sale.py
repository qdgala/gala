...
class SaleOrder(models.Model):
    ...
    hd_quote_id = fields.Many2one('hd.quote', u'华丹报价单')
    hb_type = fields.Many2one('product.product', u'类型')
    xshth = fields.Char(u'销售合同号')
    jiacheng = fields.Integer(u'销售加成')
    ...
    def onc_jiacheng(self):
        ...
        for line in self.order_line:
            if line.jiagong_fs:
                vals = {}
                vals['caiwu_danjia'] = line.yuanshi_dj * jc[line.jiagong_fs] / 100.0
                vals['caiwu_heji'] = vals['caiwu_danjia'] * line.product_uom_qty
                vals['price_unit'] = vals['caiwu_danjia'] * self.jiacheng / 100.0
                vals['price_subtotal'] = vals['price_unit'] * line.product_uom_qty
                print vals
                line.update(vals)

    @api.multi
    def btn_open_this(self):
        return {
            'name': _(self.name),
            'view_type': 'form',
            'view_mode': 'form',
    ...
    #qxs
    jh_date = fields.Date(string=u'计划发货日期')
    qr_date = fields.Date(string=u'确认发货日期')
    ...
    #qxs
    @api.multi
    def action_create_pu(self):
        ly = self.env['purchase.apply'].search([('ly', '=', self.id)])
        if ly:
            raise UserError(_(u'已经成功创建过采购申请单！'))
    ...
    def action_create_mrp(self):
        ly = self.env['mrp.apply'].search([('ly', '=', self.id)])
        if ly:
            raise UserError(_(u'已经成功创建过生产计划单！'))
        emp = self.env['hr.employee'].search([('user_id', '=', self.env.uid)], limit=1)
        vals = {
            'ly':self.id,
            'employee_id': emp.id,
        ...
        for line in self.order_line:
            if line.jiagong_fs=='zz':
                ...
                vals['order'].append([0,0,result])
        b = self.env['mrp.apply'].create(vals)
        return {
                'name': _('生产计划单'),
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'mrp.apply',
                'view_id': self.env.ref('purchase_hb.mrp_apply_form').id,
                'type': 'ir.actions.act_window',
                'res_id': b.id,
            }
...
class SaleOrderLine(models.Model):
    ...
    #qxs move_ids = fields.Many2many('stock.move', compute='_compute_move_ids', string=u'库存移动')
    #
    # @api.multi
    # @api.depends('procurement_ids.move_ids')
    # def _compute_picking_ids(self):
    #     for order in self:
    #         order.move_ids == self.procurement_ids.mapped('move_ids')

    jiagong_fs = fields.Selection([('gn', u'国内'), ('gw', u'国外'), ('zz', u'自制'), ('cgzf', u'采购直发')], string=u'加工方式')
    guige = fields.Char(u'规格')
    yuanshi_dj = fields.Float(u'原始单价')
    caiwu_danjia = fields.Float(u'财务单价')
    caiwu_heji = fields.Float(u'财务合计')

    weight=fields.Float(u'单重')
    weight_total=fields.Float(u'总重')

    # @api.onchange('product_id')
    # def onc_product_id(self):
    #     self.jiagong_fs=self.product_id.jiagong_fs
    ...
    def product_id_change(self):
        ...
        vals['jiagong_fs'] = self.product_id.jiagong_fs
        vals['yuanshi_dj'] = self.product_id.lst_price
        vals['weight']=self.product_id.weight
        vals['weight_total']=vals['weight'] * self.product_uom_qty
        vals['guige']=self.product_id.guige
        if vals.get('price_unit'):
            vals['price_subtotal']=vals['price_unit'] * self.product_uom_qty
    ...
    def product_uom_change(self):
        if not self.product_uom:
            self.price_unit = 0.0
            return
        self.weight_total = self.weight * self.product_uom_qty
        self.caiwu_heji=self.caiwu_danjia * self.product_uom_qty
