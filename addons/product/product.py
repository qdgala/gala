...
class product_template(osv.osv):
    ...
    def _get_product_template_type(self, cr, uid, context=None):
        return [('consu', _('Consumable')), ('service', _('Service')),('xt',u'系统')]
    ...
    _columns = {
        ...
        'uom_id': fields.many2one('product.uom', u'计量单位', required=True, help="Default Unit of Measure used for all stock operation."),
        'uom_po_id': fields.many2one('product.uom', u'采购计量单位', required=True, help="Default Unit of Measure used for purchase orders. It must be in the same category than the default unit of measure."),
        ...
        #qxs方便导入修改名称
        'barcode': fields.related('product_variant_ids', 'barcode', type='char', string=u'条码', oldname='ean13'),
        'default_code': fields.related('product_variant_ids', 'default_code', type='char', string=u'编码'),
        'item_ids': fields.one2many('product.pricelist.item', 'product_tmpl_id', 'Pricelist Items'),
        #qxs增加字段规格
        'guige': fields.char(u'规格'),
        'jiagong_fs': fields.selection([('gn', u'国内'),('gw',u'国外'),('zz',u'自制'),('cgzf', u'采购直发'),('gnzf', u'国内直发')], u'加工方式'),
        'default_gys' : fields.many2one('res.partner', u'默认供应商', domain=[('supplier', '=', True)], ondelete='cascade', help="Vendor of this product"),
    }
...
class product_product(osv.osv):
    ...
    _columns = {
        ...
        #qxs
        'default_code' : fields.char(u'编码', select=True),
        'active': fields.boolean('Active', help="If unchecked, it will allow you to hide the product without removing it."),
    ...
    #qxs增加产品编码约束
    _sql_constraints = [
        ('barcode_uniq', 'unique(barcode)', _(u"条码不可重复!!")),
        ('default_code_uniq', 'unique(default_code)', _(u"编码不可重复!!")),
    ]
    ...
    def name_search(self, cr, user, name='', args=None, operator='ilike', context=None, limit=100):
        ...
        if name:
            ...
            if not ids and operator not in expression.NEGATIVE_TERM_OPERATORS:
                ...
                ids = self.search(cr, user, args + ['&',('default_code', operator, name),('guige', operator, name)], limit=limit, context=context)
                ...
                ids = self.search(cr, user, args + ['&','&', ('default_code', operator, name), ('name', operator, name), ('guige', operator, name)], limit=limit, context=context)
...
class product_supplierinfo(osv.osv):
    ...
    #qxs
    def default_partner(self, cr, uid, ids, context=None):
        pro = self.pool.get('product.template')
        for i in self.browse(cr, uid, ids, context=context):
            if i.name:
                pro.write(cr, uid, [i.product_tmpl_id.id], {'default_gys':i.name.id}, context=context)
        return True
