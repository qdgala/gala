...
class PurchaseOrder(models.Model):
    ...
    def _amount_all(self):
        for order in self:
            ...
            order.update({
                ...
                'amount_total': order.currency_id.round(amount_untaxed),
            })
    #qxs计算采购订单已付未付
    @api.depends('order_line.invoice_lines.invoice_id.residual',
    ...
    def _yfk_all(self):
        for order in self:
            ...
            for i in self.invoice_ids:
                if i.state == 'paid':
                    a = i.amount_total_signed
                    b = i.residual
                    c = a - b
                    wfk += c
                    kp += a
            order.yfk_total = wfk
            order.wfk_total = order.amount_total - wfk
            order.kp_total = kp
    #qxs列表形式的预警时间
    @api.onchange('payment_term_id','amount_total')
    def _amount_term_all(self):
        if self.amount_total:
            fk_time = self.payment_term_id.compute(self.amount_total)
            new_lines = self.env['purchase.fk.line']
            if fk_time:
                for i in fk_time[0]:
                    data = {
                        'name': i[0],
                        'je': i[1],
                        'state':'draft',
                        'tj': i[2] and i[2] or False
                    }
                    new_line = new_lines.new(data)
                    new_lines += new_line
                self.fk_times = new_lines
    ...
    def _get_packing(self):
        for order in self:
            for i in order.order_line:
                if i.qty_received < i.product_qty:
                    order.packing_status = 'no'
                    continue
                else:
                    order.packing_status = 'yes'
    ...
    #qxs计算预付款笔数
    @api.multi
    def _compute_yufu(self):
        for order in self:
            payment = self.env['account.move.line']
            pay_num = payment.search([('account_id', '=', 32), ('partner_id', '=', self.env['res.partner']._find_accounting_partner(self.partner_id).id),('reconciled', '=', False), ('amount_residual', '!=', 0.0),('credit', '=', 0), ('debit', '>', 0) ])
            order.payment_count = len(pay_num)

    #qxs计算预警时间
    @api.depends('fk_times.state','fk_times.name','packing_status','invoice_status')
    def _fk_time(self):
        for order in self:
            atime = ''
            if order.fk_times:
                for i in order.fk_times:
                    if i.state == 'draft':
                        if i.tj == 'kp':
                            if order.packing_status == 'yes' and order.invoice_status == 'invoiced':
                                if atime == '':
                                    atime = i.name
                                elif atime > i.name:
                                    atime = i.name
                        elif i.tj == 'dh':
                            if order.packing_status == 'yes':
                                if atime == '':
                                    atime = i.name
                                elif atime > i.name:
                                    atime = i.name
    ...
    #qxs增加显示预付款笔数字段
    payment_count = fields.Integer(compute="_compute_yufu", string=u'预付笔数', copy=False, default=0)
    ...
    packing_status = fields.Selection([
        ('no', u'未交货'),
        ('yes', u'已交货'),
        ], string=u'交货状态', compute='_get_packing', store=True, readonly=True, copy=False, default='no')
    ...
    #qxs增加到货和付款预警时间
    fk_time = fields.Date(compute="_fk_time",string = u'付款预警时间',readonly=True, select=True, copy=False,default=0)
    dh_time = fields.Date(u'到货预警时间', select=True, states=READONLY_STATES, copy=False)
    fk_times = fields.One2many('purchase.fk.line', 'order_id', string=u'付款计划', copy=True)
    yfk_total = fields.Monetary(string=u'已付款', store=True, readonly=True, compute='_yfk_all')
    wfk_total = fields.Monetary(string=u'未付款', store=True, readonly=True, compute='_yfk_all')
    kp_total = fields.Monetary(string=u'开票金额', store=True, readonly=True, compute='_yfk_all')
    xshth1 = fields.Char(u'销售合同号')
    #qxs发通知

    @api.model
    def notice_webclient(self):
        today = time.strftime('%Y-%m-%d',time.localtime(time.time()))
        purchase = self.env['purchase.order']
        ids = purchase.search([])
        for i in ids:
            if i.fk_time and i.fk_time<today:
                vals = ({"key": u"采购单 :", "value": i.name},
                        {"key": u"供应商 :", "value": i.partner_id.name},
                        {"key": u"付款预警时间 :", "value": i.fk_time},
                        {"key": u"消息发送时间 :", "value": today})
                body = u"<h3>%s</h3>" % (u'付款预警提醒')
                for val in vals:
                    body += u"<b>%s</b>%s<br/>" % (val['key'], val['value'])
                mc = self.env['mail.channel']
                a = mc.channel_get_and_minimize(uid2pid(i, [11]))
    ...
    def action_view_payment(self):
        ...
        result = action.read()[0]
        result['context'] = {}
        pick = payment.search([('account_id', '=', 32),('partner_id', '=', self.env['res.partner']._find_accounting_partner(self.partner_id).id),('reconciled', '=', False), ('amount_residual', '!=', 0.0),('credit', '=', 0), ('debit', '>', 0)])
        pick_ids=[]
        for i in pick:
            pick_ids.append(i.id)
        if len(pick_ids) > 1:
            result['domain'] = "[('id','in',[" + ','.join(map(str, pick_ids)) + "])]"
        elif len(pick_ids) == 1:
...
class PurchaseOrderLine(models.Model):
    ...
    @api.depends('product_qty', 'price_unit', 'taxes_id', 'hsdj')
    def _compute_amount(self):
        for line in self:
            if line.hsdj:
                dj = line.hsdj/(1+(line.taxes_id.amount/100))
                taxes = line.taxes_id.compute_all(line.hsdj, line.order_id.currency_id, line.product_qty, product=line.product_id, partner=line.order_id.partner_id)
                line.update({
                    'price_unit':dj,
    ...
    def _compute_qty_invoiced(self):
        for line in self:
            ...
            for inv_line in line.invoice_lines:
                if inv_line.invoice_id.state not in ['cancel'] and inv_line.invoice_id.type in ['in_invoice']:
                    #qxs开单数量计算方法修改
    ...
    price_unit = fields.Float(string='Unit Price', required=True, group_operator = False, digits=dp.get_precision('Product Price'))
    hsdj = fields.Float(string=u'含税单价', group_operator = False, digits=dp.get_precision('Product Price'))

    price_subtotal = fields.Monetary(compute='_compute_amount', string='Subtotal', store=True)
    price_total = fields.Monetary(compute='_compute_amount', string='Total', store=True)
    price_tax = fields.Monetary(compute='_compute_amount', string='Tax', store=True)
    #qxs增加已付未付字段
    yingfk_total = fields.Monetary(related='order_id.amount_total',group_operator = 'max', string=u'订单应付款', store=True)
    yfk_total = fields.Monetary(related='order_id.yfk_total',group_operator = 'max', string=u'订单已付款', store=True)
    wfk_total = fields.Monetary(related='order_id.wfk_total',group_operator = 'max', string=u'订单未付款', store=True)
    kp_total = fields.Monetary(related='order_id.kp_total',group_operator = 'max', string=u'订单开票金额', store=True)
...
#qxs增加预警时间子表
class Product_fk_line(models.Model):
    _name = 'purchase.fk.line'
    order_id = fields.Many2one('purchase.order', string=u'采购订单')
    name = fields.Date(u'付款时间')
    je = fields.Float(string=u'付款金额', required=True, digits=dp.get_precision('Product Price'))
    tj = fields.Selection([
            ('sj', u'时间'),
            ('dh', u'到货'),
            ('kp', u'开票')
        ], string=u'付款条件',default='sj')
    state = fields.Selection([
        ('draft', u'未付'),
        ('done', u'已付'),
        ], string='状态', readonly=True, select=True, copy=False, default='draft')
    @api.multi
    def open_this_rec(self):
        if self.je:
            return {
                    ...
                    'type': 'ir.actions.act_window',
                    'target': 'new',
                    'context': "{'default_payment_type': 'outbound','default_amount': %d,'default_partner_id': %d}" % (self.je,self.order_id.partner_id.id)
                }
    @api.multi
    def fk_done(self):
        self.state = 'done'
