...
class oa_feiyongbx(models.Model):
    _name = 'oa.feiyongbx'
    _description = u'费用报销申请'
    _rec_name = 'feiyongbx_no'
    _inherit = ['ir.needaction_mixin']
    _order = 'create_date desc'

    feiyongbx_no = fields.Char(u'报销编号')
    ...
    jiekuane = fields.Float(u'原借款额', compute='_cmpt_jiekuane', store=True)
    jiekuan_id = fields.Many2one('oa.jiekuan', u'借款单号')
    zhuangtai = fields.Char(u'状态')
    shuoming = fields.Text(u'报销说明')
    mingxi = fields.Text(u'报销明细')

    sumu = fields.Char(u'单据数目')
    heji = fields.Float(u'合计金额')
    shifa = fields.Float(u'实发金额')
    ...
    @api.depends('jiekuan_id')
    def _cmpt_jiekuane(self):
        self.jiekuane = self.jiekuan_id.amount
    ...
    def onc_emp_id(self):
        ...
        self.jiekuan_id = False

    @api.model
    def create(self, vals):
        vals['feiyongbx_no'] = self.env['ir.sequence'].next_by_code('oa.feiyongbx.no')
        return super(oa_feiyongbx, self).create(vals)
    ...
    def write(self, vals):
        ...
        if vals.get('state') == 'bxr':
            if self.jiekuan_id:
                self.jiekuan_id.sudo().is_bx = True
        return super(oa_feiyongbx, self).write(vals)
    ...
    def send_message(self, state):
        ...
        vals = ({"key": u"报销编号:", "value": self.feiyongbx_no},
...
class oa_jiekuan(models.Model):
    ...
    fybx_ids = fields.One2many('oa.feiyongbx', 'jiekuan_id', u'费用报销')
